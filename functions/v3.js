...
votingContract.events.VoteCasted().on('data', event => {
    console.log('Vote Casted:', event.returnValues.voter, 'Candidate ID:', event.returnValues.candidateId);
});

votingContract.events.CandidateAdded().on('data', event => {
    console.log('Candidate Added:', event.returnValues.candidateId, 'Name:', event.returnValues.name);
});

// Function to whitelist a user
async function whitelistUser(registrationNumber, account) {
    const result = await votingContract.methods.whitelistUser(registrationNumber).send({ from: account });
    console.log('Whitelist Transaction:', result.transactionHash);
}

// Function to vote for a candidate
async function vote(candidateId, account) {
    const result = await votingContract.methods.vote(candidateId).send({ from: account });
...
async function addCandidate(name, imageURL, account) {
    const result = await votingContract.methods.addCandidate(name, imageURL).send({ from: account });
...
async function getWalletForRegistration(registrationNumber) {
    const walletAddress = await votingContract.methods.getWalletForRegistration(registrationNumber).call();
...
async function getTotalRegisteredVoters() {
    const totalVoters = await votingContract.methods.getTotalRegisteredVoters().call();
